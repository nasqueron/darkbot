...
struct setup_parameter parameters[] =
{
    ...
    {ST_STRING,  3, sizeof(REALNAME),               {"BOT_REAL_NAME",          NULL, NULL, NULL, NULL}, "bot's real name",                         REALNAME,              NULL},
    ...
    {ST_INTEGER, 3, sizeof(CONNECT_WAIT_TIMEOUT),   {"CONNECT_TIMEOUT",        NULL, NULL, NULL, NULL}, "server connection timeout",               &CONNECT_WAIT_TIMEOUT, NULL},
...
//    {ST_BOOLEAN, 3, sizeof(ALLOW_DEL_IN_MSG),       {"ALLOW_DEL_IN_MSG",       NULL, NULL, NULL, NULL}, "allowing DEL command in private",         &ALLOW_DEL_IN_MSG,     NULL},
    {ST_BOOLEAN, 3, sizeof(MSG_RESPONSES),          {"PRIVATE_QUESTIONS",      NULL, NULL, NULL, NULL}, "bot responds to private questions",       &MSG_RESPONSES,        NULL},
    {ST_BOOLEAN, 3, sizeof(LOG_ADD_DELETES),        {"LOG_ADD_DELETES",        NULL, NULL, NULL, NULL}, "logging of database changes",             &LOG_ADD_DELETES,      NULL},
    {ST_BOOLEAN, 3, sizeof(LOG_PRIVMSG),            {"LOG_PRIVMSG",            NULL, NULL, NULL, NULL}, "logging of private messages",             &LOG_PRIVMSG,          NULL},
    ...
    {ST_STRING,  3, sizeof(WAKEUP_ACTION),          {"WAKEUP_TEXT",            NULL, NULL, NULL, NULL}, "wakeup action",                           WAKEUP_ACTION,         NULL},
    ...
    {ST_STRING,  3, sizeof(EXISTING_ENTRY),         {"EXISTING_TEXT",          NULL, NULL, NULL, NULL}, "replacing existing complaint",            EXISTING_ENTRY,        NULL},
    {ST_STRING,  3, sizeof(NO_ENTRY),               {"NO_ENTRY_TEXT",          NULL, NULL, NULL, NULL}, "non-existing topic complaint",            NO_ENTRY,              NULL},
...
//    {ST_STRING,  3, sizeof(CANT_FIND),              {"CANT_FIND",              NULL, NULL, NULL, NULL}, "can't find search complaint",             CANT_FIND,             NULL},
    {ST_STRING,  3, sizeof(NO_TOPIC),               {"NO_TOPIC_TEXT",          NULL, NULL, NULL, NULL}, "no topic search complaint",               NO_TOPIC,              NULL},
    {ST_STRING,  3, sizeof(TRY_FIND),               {"TRY_FIND_TEXT",          NULL, NULL, NULL, NULL}, "try find search complaint",               TRY_FIND,              NULL},
    {ST_STRING,  3, sizeof(WHUT),                   {"WHUT_TEXT",              NULL, NULL, NULL, NULL}, "no text complaint",                       WHUT,                  NULL},
    {ST_BOOLEAN, 3, sizeof(RANDOM_WHUT),            {"RANDOM_WHUT_TEXT",       NULL, NULL, NULL, NULL}, "random no text complaint",                &RANDOM_WHUT,          NULL},
    {ST_STRING,  3, sizeof(DUNNO_Q),                {"DUNNO_TEXT",             NULL, NULL, NULL, NULL}, "no answer complaint",                     DUNNO_Q,               NULL},
    {ST_BOOLEAN, 3, sizeof(RANDOM_DUNNO),           {"RANDOM_DUNNO_TEXT",      NULL, NULL, NULL, NULL}, "random no answer complaint",              &RANDOM_DUNNO,         NULL},
#ifdef ENABLE_RANDOM
...
//    {ST_BOOLEAN, 3, sizeof(RANDOM_STUFF),           {"RANDOM_STUFF",           NULL, NULL, NULL, NULL}, "random utterences",                       &RANDOM_STUFF,         NULL},
    ...
    {ST_INTEGER, 3, sizeof(RAND_IDLE),              {"RAND_IDLE_TIME",         NULL, NULL, NULL, NULL}, "seconds idle beforerandom utterences",    &RAND_IDLE,            NULL},
...
//    {ST_BOOLEAN, 3, sizeof(RANDQ),                  {"ALLOW_RANDQ",            NULL, NULL, NULL, NULL}, "enable RANDQ command",                    &RANDQ,                NULL},
    ...
    {ST_STRING,  3, sizeof(COMPLAIN_REASON),        {"COMPLAIN_TEXT",          NULL, NULL, NULL, NULL}, "bot kicked message",                      COMPLAIN_REASON,       NULL},
    ...
    {ST_BOOLEAN, 3, sizeof(JOIN_GREET),             {"USER_GREET_TEXT",        NULL, NULL, NULL, NULL}, "registered user greeting",                &JOIN_GREET,           NULL},
    {ST_BOOLEAN, 3, sizeof(HELP_GREET),             {"NEW_GREET_TEXT",         NULL, NULL, NULL, NULL}, "give new users help",                     &HELP_GREET,           NULL},
    {ST_BOOLEAN, 3, sizeof(AUTOHELP_GUESTS),        {"GUESTS_GCREET_TEXT",     NULL, NULL, NULL, NULL}, "give guests help",                        &AUTOHELP_GUESTS,      NULL},
